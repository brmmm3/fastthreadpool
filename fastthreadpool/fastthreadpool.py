...
class Semaphore(object):  #p
    ...
    #c def __cinit__(self, name, int value=1):
    def __init__(self, name, value=1):  #p
        if value < 0:
...
class Pool(object):  #p
    ...
    def __init__(self, max_children=-9999, child_name_prefix="", init_callback=None,  #p
        ...
        self._done = deque()
        self._failed = deque()
        self._done_cnt = Semaphore("done", 0)
        self._failed_cnt = Semaphore("failed", 0)
